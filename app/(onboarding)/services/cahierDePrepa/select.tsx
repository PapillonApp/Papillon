...
export default function CahierDePrepaSelect() {
  ...
  const selectSchool = (school: School) => {
    navigation.navigate("browser" as never, { url: school.url, school } as never);
  };
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.overground }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.select({ android: 0, default: 20 })}>
      <List
        animated
        ListHeaderComponent={<CahierDePrepaSearchHeader search={search} setSearch={setSearch} loading={loading} t={t} />}
