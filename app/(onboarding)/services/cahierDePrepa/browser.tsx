...
import { setPendingPronoteAuth } from "@/stores/account/pendingCombinedLogin";
...
export default function CahierDePrepaBrowser() {
  ...
  const onWebviewMessage = async ({ nativeEvent }: { nativeEvent: WebViewMessage }) => {
    ...
    if (message.type === "pronote.loginState") {
      ...
      console.log("Pronote login credentials received securely");
      ...
      try {
        ...
        // Store Pronote auth in memory and redirect to Cahier de Prépa WebView
        setPendingPronoteAuth({
          auth,
          firstName,
          lastName,
          schoolName,
          className,
          customisation: { profilePicture: pp, subjects: {} },
        });

        router.replace("./webview" as never);
      } catch (error) {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding" keyboardVerticalOffset={20}>
      ...
      <OnboardingWebView
        ...
        onOpenWindow={(evt) => {
          ...
          webViewRef.current?.injectJavaScript(`window.location.assign("${evt.nativeEvent.targetUrl}");`);
        }}
        injectedJavaScript={`
