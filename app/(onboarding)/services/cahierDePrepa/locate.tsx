...
const CahierDePrepaSearchHeader = memo(({
  city,
  setCity,
  loading,
  showElse,
...
export default function CahierDePrepaLocate() {
  ...
  const selectCity = (city: School) => {
    navigation.navigate(`select` as never, { city: city } as never);
  };
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.overground }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.select({ android: 0, default: 20 })}>
      <List
        ListHeaderComponent={<CahierDePrepaSearchHeader city={city} setCity={setCity} loading={loading && cities.length === 0} showElse={cities.length === 0 && !loading} t={t} />}
      ...
      >
        {cities.length === 0 && !loading && (
          <List.Item animated onPress={() => navigation.navigate("qrcode" as never)}>
            <List.Leading>
        ...
        {cities.length === 0 && !loading && (
          <List.Item animated onPress={() => navigation.navigate("url" as never)}>
            <List.Leading>
