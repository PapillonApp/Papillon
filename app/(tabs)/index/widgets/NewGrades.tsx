...
const HomeNewGradesWidget = React.memo(() => {
  ...
  const fetchGradesForPeriod = async (
    ...
    if (period && managerToUse) {
      const grades = await managerToUse.getGradesForPeriod(
        period,
  ...
  if (loading) {
    return (
      ...
      >
        <ActivityIndicator size={30} strokeWidth={3} color={colors.primary} />
      </Dynamic>
    );
  }
...
export default HomeNewGradesWidget;
