...
export function useTimetableData(
  ...
  const rawTimetable = useTimetable(
    refresh,
  ...
  const fetchWeeklyTimetable = useCallback(
    async (targetWeekNumber: number, forceRefresh = false) => {
      setIsLoading(true);
      ...
      fetchTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          const candidates = [
            ...
            targetDate.setDate(
              targetDate.getDate() + (week - targetWeekNumber) * 7
            );
          ...
          if (toFetch.length > 0) {
            await Promise.all(
              toFetch.map(async c => {
                return await manager.getWeeklyTimetable(c.week, c.targetDate);
              })
            );
