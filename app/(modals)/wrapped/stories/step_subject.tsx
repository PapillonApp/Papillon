...
export const StepSubject = ({ isCurrent }: { isCurrent: boolean, sliderRef: React.RefObject<FlatList> }) => {
  const { topSubject, topSubjectCount, topRoom, topTeacher } = useWrappedStats();
  ...
  return (
    <View style={{ width: "100%", height: Dimensions.get('screen').height, justifyContent: 'center', alignItems: 'center', backgroundColor: "#00000075" }}>
      {isCurrent && (
        <>
          {step >= 0 && (
            <Stack animated layout={LinearTransition.springify().duration(1000)} gap={6} vAlign='center' hAlign='center' style={{ width: Dimensions.get('screen').width - 60, ...SHADOW_OVER_ANIMATED_BG }}>
              <Reanimated.View
              ...
              >
                ...
                <Reanimated.View
                  ...
                  key={step >= 1 ? "emoji" : "icon"}
                >
                  {step >= 1 ? (
                    <Text style={{ fontSize: 72 }}>
                      {getSubjectEmoji(topSubject || "")}
                    </Text>
                  ) : (
                    <Papicons name="calendar" size={48} color='white' />
                  )}
                </Reanimated.View>
              </Reanimated.View>
              <Reanimated.View
              ...
              >
                <Typography color='white' variant='h2' align='center'>
                  Parmi tout ça, il y a ton petit préféré...
                </Typography>
              </Reanimated.View>
              ...
              {step >= 1 && (
                ...
                >
                  <Typography weight='bold' color='white' variant='h4' align='center'>
                    {getSubjectName(topSubject || "")}
                  </Typography>
                </Reanimated.View>
              )}
