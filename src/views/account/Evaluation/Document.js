...
import { MessageSquareMore, User2, } from "lucide-react-native";
import InsetsBottomView from "@/components/Global/InsetsBottomView";
import { SkillLevelBadge } from "@/views/account/Evaluation/Atoms/SkillLevelBadge";
var EvaluationDocument = function (_a) {
    var route = _a.route, navigation = _a.navigation;
    var _b = route.params, evaluation = _b.evaluation, _c = _b.allEvaluations, allEvaluations = _c === void 0 ? [] : _c;
    var theme = useTheme();
    var _d = useState({
        color: "#888888",
    ...
    var _e = useState([]), skillPerDomain = _e[0], setSkillPerDomain = _e[1];
    ...
    useEffect(function () {
        var skillsPerDomain = evaluation.skills.reduce(function (acc, skill) {
            var domainName = skill.domainName;
            var domainIndex = acc.findIndex(function (item) { return item.domainName === domainName; });
            if (domainIndex === -1) {
                acc.push({
    ...
    return (<View style={{
      ...
      <ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} style={{
        ...
        <View style={{
          ...
          {skillPerDomain.map(function (domain, index) { return (<View key={index}>
              <NativeListHeader label={domain.domainName}/>
              <NativeList>
                {domain.skills.map(function (skill, index) { return (<NativeItem key={index} leading={<SkillLevelBadge skillLevel={skill.level}/>} trailing={<NativeText variant="subtitle" style={{ fontSize: 18, marginLeft: 10 }}>
                      {skill.pillarPrefixes.join(", ")}
                    </NativeText>}>
                    <NativeText variant={"title"}>{skill.itemName}</NativeText>
                    <NativeText variant={"subtitle"}>{"Coefficient x".concat(skill.coefficient.toFixed(2))}</NativeText>
                  </NativeItem>); })}
              </NativeList>
            </View>); })}
          <NativeListHeader label={"Détails"}/>
          <NativeList>
            <NativeItem icon={<User2 />}>
              <NativeText variant="overtitle">Déposé par </NativeText>
              <NativeText variant={"subtitle"}>
                {evaluation.teacher}
              </NativeText>
            </NativeItem>
            {evaluation.description !== "" && (<NativeItem icon={<MessageSquareMore />}>
