...
import { View, ScrollView, Image, Switch, Platform, Modal, } from "react-native";
import React from "react";
import ButtonCta from "@/components/FirstInstallation/ButtonCta";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { BadgeX, Calendar, Camera, Carrot, Clock, Code, Cog, Folder, Globe, Images, LockKeyhole, MapPin, Newspaper, PieChart, Terminal, TriangleAlert, User, UserCog, } from "lucide-react-native";
import { get_addons_list } from "@/addons/addons";
import { PressableScale } from "react-native-pressable-scale";
import * as Linking from "expo-linking";
import * as FileSystem from "expo-file-system";
import { useAlert } from "@/providers/AlertProvider";
var SettingsAddons = function () {
    var _a = React.useState(false), opened = _a[0], setOpened = _a[1];
    var _b = React.useState([]), storageAddons = _b[0], setStorageAddons = _b[1];
    var _c = React.useState({
        ...
        icon: "",
        error: ""
    }), selectedAddons = _c[0], setSelectedAddons = _c[1];
    React.useEffect(function () {
        get_addons_list().then(function (r) {
            setStorageAddons(r);
        });
    }, []);
    ...
    return (<View>
      <Modal animationType="slide" visible={opened} onRequestClose={function () { return setOpened(false); }} presentationStyle="pageSheet">
        <View style={{
        ...
        <ScrollView style={{
          ...
          <View style={{ display: "flex", gap: 20, paddingTop: 23 }}>
            ...
            paddingHorizontal: 16
              ...
              <Image source={selectedAddons.icon == "" ? require("../../../assets/images/addon_default_logo.png") : { uri: selectedAddons.icon }} style={{
            width: 90,
            height: 90,
            borderRadius: 20,
            borderWidth: 1,
            ...
            <View>
              ...
              <ScrollView horizontal={true} style={{ marginTop: 10 }} snapToInterval={130} showsHorizontalScrollIndicator={false}>
                <View style={{ display: "flex", gap: 10, flexDirection: "row", paddingHorizontal: 16 }}>
                  {selectedAddons.screenshot.map(function (screenshot, index) { return (<Image key={index} source={{ uri: screenshot }} style={{
                width: 120,
                height: 250,
                borderRadius: 10,
                borderWidth: 1,
            ...
            <View style={{ paddingHorizontal: 16 }}>
              ...
              <NativeList style={{ marginTop: 0 }}>
                {selectedAddons.permissions.map(function (permission, index) { return (<NativeItem key={"perm_" + index} leading={permission.name == "PERM_APP_CAMERA" ? <Camera color={"#000"} size={24}/> :
                ...
                {selectedAddons.domains.map(function (domain, index) { return (<NativeItem key={"domain_" + index} leading={<Globe size={24} color={"#000"}/>}>
      ...
      <ScrollView style={{
        ...
        <NativeList>
          {storageAddons.map(function (addon, index) { return (<NativeItem key={index} onPress={addon.error ? function () { } : function () {
                setSelectedAddons(addon);
                setOpened(true);
            }} leading={<Image source={addon.icon == "" ? require("../../../assets/images/addon_default_logo.png") : { uri: addon.icon }} style={{
                    width: 46,
                    height: 46,
                    borderRadius: 10,
                    borderWidth: 1,
                ...
                }}/>} trailing={addon.error && (<PressableScale onPress={function () {
                    var _a;
                    showAlert({
                        title: "Impossible de charger le plugin \"".concat(addon.name, "\""),
                        message: (_a = addon.error) !== null && _a !== void 0 ? _a : "Erreur inconnue",
                        icon: <BadgeX />,
                    });
                }}>
                  <TriangleAlert size={24} color={"#BE0B00"} style={{ marginRight: 5 }}/>
                </PressableScale>)}>
          ...
          {Platform.OS == "ios" && (<NativeItem leading={<Folder size={24} color={"#000"}/>} onPress={function () {
                var _a;
                Linking.openURL(((_a = FileSystem.documentDirectory) === null || _a === void 0 ? void 0 : _a.replace("file", "shareddocuments")) + "addons");
            }}>
                <NativeText variant="title">
        ...
        <NativeList>
          <NativeItem leading={<Image source={require("../../../assets/images/addon_default_logo.png")} style={{
                width: 46,
                height: 46,
                borderRadius: 10,
                borderWidth: 1,
        ...
        <NativeList>
          <NativeItem trailing={<Switch></Switch>}>
            <NativeText variant="title">
