    ...
    (error) => {
      if (
        error.response?.data?.errors?.find((e: any) =>
          e.title.includes("PRONOTE_RESOURCES")
        )
      )
      ...
      error.response?.data?.errors?.forEach((e: any) => {
        showAlert({
          ...
          message:
            htmlDecode(
              e["detail"]?.toString().replace(/<(\/)?([a-z0-9]+)>/g, "") ||
                "Erreur inconnue"
            ) +
...
export const refreshSkolengoToken = async (
  refreshToken: string,
  discovery: DiscoveryDocument
): Promise<SkolengoTokenSet> => {
  ...
  return await fetch(discovery.tokenEndpoint, {
    ...
    headers: {
      Authorization:
        "Basic " + b64encode(OID_CLIENT_ID + ":" + OID_CLIENT_SECRET),
    },
    body: formData,
  })
    .then((response) => response.json())
    .then((d) => authTokenToSkolengoTokenSet(d));
};

const getJWTClaims = (token: string): SkolengoJWT => {
  const dataPart = token
    .split(".")
    ?.at(1)
    ?.replace(/-/g, "+")
    .replace(/_/g, "/");
...
export const getSkolengoAccount = async (
  authConfig: SkolengoAuthConfig,
  userInfo?: User
) => {
  const skolengoAccount = new Skolengo(
    ...
    authConfig.tokenSet,
    {
      refreshToken: async (tokenSet): Promise<SkolengoTokenSet> => {
        ...
        return refreshSkolengoToken(
          tokenSet.refresh_token,
          authConfig.discovery
        );
      },
      onTokenRefresh: async (tokenSet) => {
        const state = useCurrentAccount.getState().account
          ?.authentication as SkolengoAccount["authentication"];
